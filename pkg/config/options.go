...
import (
	...
	"github.com/khulnasoft-lab/YaraSec/utils"
)

var (
	product string = utils.GetEnvOrDefault("KHULNASOFT_PRODUCT", "ThreatMapper")
	license string = utils.GetEnvOrDefault("KHULNASOFT_LICENSE", "")
)
...
func ParseOptions() (*Options, error) {
	options := &Options{
		...
		ConfigPath:           flag.String("config-path", "", "Searches for config.yaml from given directory. If not set, tries to find it from YaraSec binary's and current directory"),
		...
		Product:              flag.String("product", product, "Khulnasoft Product type can be ThreatMapper or ThreatStryker, also supports env var KHULNASOFT_PRODUCT"),
		License:              flag.String("license", license, "TheratMapper or ThreatStryker license, also supports env var KHULNASOFT_LICENSE"),
	}
	flag.Parse()
	return options, nil
}

// NewDefaultOptions returns the default options for the YaraSec without flag parsing
